...
library(tidyr)
library(stringr)
raw_data<-read.csv("~/explaining atheism/ariel_project/cleaned_data_nonbeliever.csv",fileEncoding="utf8")
...
combined_long$Type <- recode(combined_long$Type, 
                             'Belief' = 'Belief change (field nonbeliever)',
                             'Behavior' = 'Behavior change (field nonbeliever)')

# Recode the 'Type' factor to have the new labels 'Belief Change' and 'Behavior Change'
combined_long$Type <- factor(combined_long$Type, levels = c("Belief change (field nonbeliever)", "Behavior change (field nonbeliever)"))
