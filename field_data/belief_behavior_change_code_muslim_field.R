...
library(tidyr)
library(stringr)
raw_data<-read.csv("~/explaining atheism/ariel_project/cleaned_data_muslim.csv",fileEncoding="utf8")
...
combined_long$Type <- recode(combined_long$Type, 
                             'Belief' = 'Belief change (field muslim)',
                             'Behavior' = 'Behavior change (field muslim)')

# Recode the 'Type' factor to have the new labels 'Belief Change' and 'Behavior Change'
combined_long$Type <- factor(combined_long$Type, levels = c("Belief change (field muslim)", "Behavior change (field muslim)"))
...
# construct a "bayesian sensitivity variable"
raw_data$bayesian_sensitivity<-raw_data$`illness low survival success (belief)`-raw_data$`illness high survival success (belief)`
# transform it into a binary variable
raw_data$bayesian_sensitivity_binary <- ifelse(raw_data$bayesian_sensitivity > 0, 1, 0)

#see if religious activity frequency predict bayesian sensitivity
# Run logistic regression
logistic_model <- glm(bayesian_sensitivity_binary ~ religious_activity_freq, 
                      data = raw_data, 
                      family = binomial)
...
#construct a variable to capture the extent people estimate belief increase
#in case of prayer failure; the average of 4 prayer failure scenarios to capture
# Bayesian rationality (negative value means rational)
raw_data$directional_rational<-(raw_data$`exam failure (belief)`+raw_data$`safe travel failure (belief)`+
  raw_data$`illness low survival failure (belief)`+raw_data$`illness high survival failure (belief)`)/4

#see if this "directional_rational" is negatively associated with education

# Run linear regression
linear_model <- lm(directional_rational ~ age + education + sex, 
                   data = raw_data)
