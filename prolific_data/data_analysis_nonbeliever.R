...
raw_data<-read.csv("~/explaining atheism/ariel_project/prolific_survey/merged_prolific_data_nonbeliever.csv",fileEncoding="utf8")
...
combined_long$Type <- recode(combined_long$Type, 
                             'Belief' = 'Belief Change (prolific nonbeliever)',
                             'Behavior' = 'Behavior Change (prolific nonbeliever)')
combined_long$Type <- factor(combined_long$Type, levels = c("Belief Change (prolific nonbeliever)", "Behavior Change (prolific nonbeliever)"))
...
model <- lm(directional_bayesian_rationality ~ Age + education + religiosity, data = raw_data)
