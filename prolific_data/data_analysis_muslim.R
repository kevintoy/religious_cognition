...
raw_data<-read.csv("~/explaining atheism/ariel_project/prolific_survey/merged_prolific_data_muslim.csv",fileEncoding="utf8")
...
combined_long$Type <- recode(combined_long$Type, 
                             'Belief' = 'Belief Change (prolific muslim)',
                             'Behavior' = 'Behavior Change (prolific muslim)')
combined_long$Type <- factor(combined_long$Type, levels = c("Belief Change (prolific muslim)", "Behavior Change (prolific muslim)"))
