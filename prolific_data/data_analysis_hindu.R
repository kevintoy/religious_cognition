...
raw_data<-read.csv("~/explaining atheism/ariel_project/prolific_survey/merged_prolific_data_hindu.csv",fileEncoding="utf8")
...
combined_long$Type <- recode(combined_long$Type, 
                             'Belief' = 'Belief Change (prolific hindu)',
                             'Behavior' = 'Behavior Change (prolific hindu)')
combined_long$Type <- factor(combined_long$Type, levels = c("Belief Change (prolific hindu)", "Behavior Change (prolific hindu)"))
